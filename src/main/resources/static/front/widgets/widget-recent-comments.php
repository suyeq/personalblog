...
class Memory_widget_recent_comments extends WP_Widget{
	...
	function widget( $args, $instance ){
		...
		echo $args['before_widget'];
			if( !empty( $title ) ) echo '<header class="art-widget-header"><h3 class="recent-comments">' . $title . '</h3></header>';
...
?>
						<li>
							<a href="<?php echo esc_url( get_comment_link( $comment ) ); ?>" title="<?php echo get_comment_author( $comment ); ?>">
								...
								<div class="right-box">
									<span class="author"><?php echo get_comment_author( $comment ); ?></span>于<span><?php echo get_comment_date('n-d H:i', $comment->comment_ID); ?></span>在<span class="title"><?php echo esc_attr( $a_title ); ?></span>说: <br/><span class="recent-comment"><?php echo htmlspecialchars( get_comment_text( $comment ) ); ?></span>
								</div>
							</a>
						</li>
<?php
...
?>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'exclude_posts' ) ); ?>"><?php _e( '排除文章：', 'Memory' ); ?></label>
